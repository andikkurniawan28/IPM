            ...
            <div class="card-body">
                <form action="{{ route('parameter.update', $parameter->id) }}" method="POST">
                        ...
                        <select name="kategori_parameter_id" id="kategori_parameter_id" class="form-select" required>
                            ...
                            @foreach ($kategori_parameters as $kategori)
                                <option value="{{ $kategori->id }}"
                                    {{ $parameter->kategori_parameter_id == $kategori->id ? 'selected' : '' }}>
                                    {{ $kategori->nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="satuan_id" id="satuan_id" class="form-select" required>
                            ...
                            @foreach ($satuans as $satuan)
                                <option value="{{ $satuan->id }}"
                                    {{ $parameter->satuan_id == $satuan->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="keterangan" id="keterangan" class="form-control" rows="3">{{ $parameter->keterangan }}</textarea>
                    </div>
